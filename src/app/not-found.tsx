  ...
  return (
    <section className="relative overflow-hidden -mt-[72px] pt-[72px]">
      <div
        ...
        style={{
          background: `
            radial-gradient(ellipse 75% 55% at 18% 0%, #f7fee7 0%, transparent 55%),
            radial-gradient(ellipse 65% 60% at 88% 25%, #ecfccb 0%, transparent 60%),
            radial-gradient(ellipse 80% 50% at 50% 95%, #dcfce7 0%, transparent 65%)
          `,
          opacity: 0.55,
          filter: 'blur(56px)',
        }}
      ...
      <Container className="text-center py-24 md:py-32">
        ...
        <div className="flex flex-col sm:flex-row gap-3.5 justify-center items-center">
          <Link
            href="/"
            className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#64a028] hover:bg-[#84cc16] text-white font-bold text-base rounded-full transition-colors tracking-tight shadow-[0_8px_24px_rgba(132,160,40,0.28),inset_0_1px_0_rgba(255,255,255,0.20)]"
          >
...
function QuickLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="px-4 py-2 bg-white/60 border border-[rgba(15,19,34,0.10)] hover:border-[#64a028] hover:text-[#4a7818] rounded-full text-sm font-medium text-[rgba(15,19,34,0.78)] transition-colors backdrop-blur-sm tracking-[-0.005em]"
    >
      {children}
    </Link>
  );
