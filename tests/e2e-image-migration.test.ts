...
const network = process.env.POPGAS_WEBSITE_MIGRATION_NETWORK ?? `popgas-website-migration-${suffix}`;
const database = process.env.POPGAS_WEBSITE_MIGRATION_DATABASE ?? `popgas-website-postgres-${suffix}`;
const owner = process.env.POPGAS_WEBSITE_MIGRATION_OWNER ?? `popgas-website-migration-contract-${suffix}`;
